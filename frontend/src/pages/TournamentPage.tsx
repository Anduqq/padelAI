    ...
    <section className="podium-panel confetti-panel">
      ...
      <article className="podium-hero">
        ...
        <div className="podium-hero-copy">
          ...
          <span>{champion.points} pts | {champion.wins}W {champion.losses}L</span>
        </div>
      </article>
...
function KnockoutResultCard({
  ...
  title: string;
  match: { team_a_label: string; team_b_label: string; team_a_score: number | null; team_b_score: number | null };
  accent?: "gold" | "bronze";
}) {
  const winner = winningLabel(match);
  return (
    <article className={`knockout-result-card knockout-result-card-${accent}`}>
      <div className="split-row">
        <div>
          <p className="eyebrow">{title}</p>
          <h4>{winner ?? "Winner pending"}</h4>
        </div>
        <span className="status-badge status-completed">{winner ? "Played" : "Pending"}</span>
      </div>
      <div className="knockout-result-team"><span>{match.team_a_label}</span><strong>{match.team_a_score ?? "-"}</strong></div>
      <div className="knockout-result-team"><span>{match.team_b_label}</span><strong>{match.team_b_score ?? "-"}</strong></div>
    </article>
  );
}
...
function BracketGraph({ graph }: { graph: TournamentDetail["bracket_graph"] }) {
  ...
  const mainStages = hasBronzeMatch ? [...graph.slice(0, -1), { ...lastStage, matches: lastStage.matches.slice(0, 1) }] : graph;
  ...
  mainStages.forEach((stage, stageIndex) => {
    if (stageIndex === 0) {
      stageLayouts.push(stage.matches.map((_, matchIndex) => {
        const topY = 40 + matchIndex * (blockHeight + matchGap);
        return { topY, centerY: topY + blockHeight / 2 };
      }));
      return;
    }
    ...
    stageLayouts.push(stage.matches.map((_, matchIndex) => {
      ...
      return { topY: centerY - blockHeight / 2, centerY };
    }));
  });
  const svgHeight = Math.max(blockHeight + 80, ...stageLayouts.flat().map((layout) => layout.topY + blockHeight)) + 48;
  ...
  return (
    <section className="panel">
      <div className="split-row">
        <div>
          <p className="eyebrow">Bracket board <span role="img" aria-label="bracket">{BRACKET_ICON}</span></p>
          <h3>Knockout tree</h3>
        </div>
      </div>
          ...
          {mainStages.map((stage, stageIndex) => {
            ...
            return (
              <g key={stage.round_id}>
                <text x={stageX} y={22} className="bracket-stage-title">{stage.title}</text>
                ...
                {stage.carryover_label ? <g><rect x={stageX} y={svgHeight - 66} width={boxWidth} height={44} rx={14} className="bracket-bye-box" /><text x={stageX + 14} y={svgHeight - 38} className="bracket-bye-label">Bye: {stage.carryover_label}</text></g> : null}
              </g>
            );
          })}
          <g>
            <rect x={championX} y={championCenterY - 42} width={210} height={84} rx={18} className="bracket-champion-box" />
            <text x={championX + 105} y={championCenterY - 12} textAnchor="middle" className="bracket-champion-title">Champion</text>
            <text x={championX + 105} y={championCenterY + 4} textAnchor="middle" className="bracket-champion-name">{teamLabelLines(championLabel).map((line, lineIndex) => <tspan key={`champion-${lineIndex}`} x={championX + 105} dy={lineIndex === 0 ? 0 : 15}>{line}</tspan>)}</text>
          </g>
        </svg>
      </div>
      <div className="knockout-result-grid">
        <KnockoutResultCard title="Final" match={finalMatch} accent="gold" />
        {bronzeMatch ? <KnockoutResultCard title="Bronze match" match={bronzeMatch} accent="bronze" /> : null}
      </div>
    </section>
  );
}
    ...
    <div className="stack-section">
      <section className={`panel ${tournament.status === "completed" ? "confetti-panel" : ""}`}>
        ...
        <div className="stat-grid">
          <div className="stat-card"><span className="stat-label">Players</span><strong className="stat-value">{tournament.participants.length}</strong></div>
          <div className="stat-card"><span className="stat-label">Courts</span><strong className="stat-value">{tournament.court_count}</strong></div>
          <div className="stat-card"><span className="stat-label">Rounds target</span><strong className="stat-value">{tournament.target_rounds ?? "Auto"}</strong></div>
          <div className="stat-card"><span className="stat-label">Scoring</span><strong className="stat-value">{scoringDescription}</strong></div>
          <div className="stat-card"><span className="stat-label">Started</span><strong className="stat-value">{formatDate(tournament.started_at)}</strong></div>
        </div>
        ...
        <div className="split-row">
              ...
              {tournament.participants.map((participant) => (
                <span key={participant.player_id} className="chip chip-static">
                  <span className="player-row">
                    <AvatarBadge name={participant.display_name} seed={participant.player_id} avatarUrl={participant.avatar_url} size="sm" />
          ...
          <div className="action-row">
            {tournament.status === "draft" ? <button type="button" className="primary-button" onClick={() => startMutation.mutate()}>{startMutation.isPending ? "Starting..." : "Start tournament"}</button> : null}
            {tournament.status === "active" ? (
              <button
                type="button"
                className="ghost-button"
                disabled={finishTournamentMutation.isPending || nextRoundMutation.isPending || continueAmericanoMutation.isPending || startBracketMutation.isPending || continueBracketMutation.isPending || scoreMutation.isPending}
                onClick={() => finishTournamentMutation.mutate()}
              >
                {finishTournamentMutation.isPending ? "Finishing..." : "Finish tournament"}
              </button>
            ) : null}
            {tournament.status === "completed" ? <button type="button" className="secondary-button" onClick={() => openTournamentWhatsAppShare(tournament)}>Share on WhatsApp</button> : null}
            {tournament.can_generate_next_round ? <button type="button" className="primary-button" onClick={() => nextRoundMutation.mutate()}>{nextRoundMutation.isPending ? "Generating..." : "Generate next Mexicano round"}</button> : null}
          </div>
        </div>
      </section>
      ...
      {tournament.status === "active" && !activeRound ? (
        <section id="tournament-actions" className="panel action-panel">
          ...
          <p className="muted-text">Finish the tournament now, continue the Americano rotation, move into the next Mexicano round, or start a seeded bracket built from the standings.</p>
          <div className="action-row">
            <button
              type="button"
              className="primary-button"
              disabled={finishTournamentMutation.isPending || continueAmericanoMutation.isPending || nextRoundMutation.isPending || startBracketMutation.isPending || continueBracketMutation.isPending}
              onClick={() => finishTournamentMutation.mutate()}
            >
              {finishTournamentMutation.isPending ? "Finishing..." : "Finish tournament"}
            </button>
                  ...
                  {round.matches.map((match) =>
                    round.status === "active" ? (
                      <div id={`match-${match.id}`} key={match.id}>
                        <ScoreEditor
                          match={match}
                          disabled={scoreMutation.isPending || unlockRoundMutation.isPending || finishTournamentMutation.isPending || continueAmericanoMutation.isPending || startBracketMutation.isPending || continueBracketMutation.isPending}
                          scoringSystem={tournament.scoring_system}
                          americanoPointsTarget={tournament.americano_points_target}
                          onSubmit={(payload) => scoreMutation.mutate({ matchId: match.id, ...payload })}
                        />
                      </div>
                    ) : (
                      <div key={match.id} className="score-summary">
                        <div>
                          ...
                          <p className="muted-text">{match.team_a.map((item) => item.display_name).join(" / ")} vs {match.team_b.map((item) => item.display_name).join(" / ")}</p>
                        </div>
                        <strong>{match.team_a_games ?? "-"} : {match.team_b_games ?? "-"}</strong>
                      </div>
                    )
                  )}
                </div>
